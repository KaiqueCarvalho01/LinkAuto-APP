      ...
      <div className="px-5 -mt-8 relative z-10 space-y-6">
        <section>
          <div className="flex justify-between items-center mb-4 px-1">
            <h3 className="font-bold text-gray-800 text-base italic">{pendingLabel}</h3>
          ...
          <div className="space-y-4">
            {requests.length > 0 ? (
              requests.map((req) => {
                const displayName = req.studentName ?? req.name ?? "Usuário";
                return (
                  <div
                    key={req.id}
                    className="bg-white rounded-[28px] p-5 shadow-xl shadow-blue-900/5 border border-gray-50"
                  >
