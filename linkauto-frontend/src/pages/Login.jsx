...
const Login = ({ onAuthenticate }) => {
	...
	const handleSubmit = async (e) => {
		...
		setErrorMessage("");
		try {
			await onAuthenticate({ email, password, preferredRole: role });
		} catch (error) {
			setErrorMessage(
				error.message || "Não foi possível autenticar sua conta.",
			);
		} finally {
			setSubmitting(false);
		}
			...
			<div className="px-8 -mt-10">
				<div className="bg-white rounded-[32px] p-8 shadow-xl shadow-blue-900/5">
					...
					<form onSubmit={handleSubmit} className="space-y-5">
						...
						{errorMessage ? (
							<p className="text-red-500 text-xs font-semibold text-center pt-1">
								{errorMessage}
							</p>
						) : null}
					</form>
